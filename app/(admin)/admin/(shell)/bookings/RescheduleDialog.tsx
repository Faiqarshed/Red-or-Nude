...
import { rescheduleBooking, type Result } from "./actions";
...
export default function RescheduleDialog({
  ...
  onDone: () => void;
  /**
   * Which action actually performs the move. Defaults to the ordinary one.
   *
   * The no-show queue passes its own, because putting a missed appointment back
   * on the calendar has to un-miss it too — see rescheduleNoShow. The picker is
   * the same question either way, so it is the same picker.
   */
  submit?: (input: { id: string; startsAt: string }) => Promise<Result>;
}) {
  ...
  const move = async (startsAt: string) => {
    ...
    const res = await submit({ id: booking.id, startsAt });
